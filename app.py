...
with tab2:
    ...
    if pwd == "admin1234": 
        ...
        if df_inc.empty:
            ...
            # 📥 ปุ่มดาวน์โหลดไฟล์บันทึกประวัติความผิดแบบ Excel
            try:
                with open(INCIDENT_FILE, "rb") as file_inc:
                    st.download_button(
                        label="📥 ดาวน์โหลดไฟล์ประวัติการทำผิดกฎระเบียบ (Excel)",
                        data=file_inc,
                        file_name=f"ประวัติทำผิดกฎจราจร_วก_ชัยภูมิ_{datetime.now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="btn_download_inc"
                    )
            except:
                pass
